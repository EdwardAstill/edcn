...
Install into an initialized shadcn app (after publishing the registry changes):
...
import type { WorkspaceViewProps } from "@/hooks/workspace/use-workspace";
import { useViewWorkingState } from "@/hooks/workspace/use-view-working-state";
...
Set these optional properties on each view in `createInitialState`, or pass
them when creating a view with `view/open`, `view/float`, or `stack/split`:

```tsx
createInitialState({
  views: [{ type: "editor", title: "Editor", closable: false, pinned: true }],
});

// Create a floating view that stays floating.
dispatch({ type: "view/float", viewType: "clock", pinned: true });
```

- `closable` defaults to `true`. Set it to `false` to hide the tab's close
  button and block workspace close commands. A floating window containing
  any non-closable tab also cannot be closed through its frame.
- `pinned` defaults to `false`. Set it to `true` to prevent changing surface
  kind: tiled views cannot float or pop out, and floating views cannot dock
  or drop into the tiled layout. Moves and splits within the tiled layout
  remain available, as do floating window movement and resizing. Pinning
  does not prevent closing; use `closable: false` for that.

Both properties persist with workspace state and are enforced by the reducer,
including keyboard commands and drag-and-drop. Native browser-window closure
still closes a popout and cleans up its views.
...
| `workspace` | block | The full `<Workspace>` block: tiled, floating, popouts, keymap |

## Architecture

```text
registry/workspace/components/  <Workspace> — provider + surfaces (tiled, floating, popouts)
registry/workspace/ui/          placement-agnostic primitives (stack, tabs, layout, handles)
registry/workspace/hooks/       React bindings (store, keymap, drag, per-view state)
registry/workspace/lib/
...
```ts
import { serializeWorkspaceState, deserializeWorkspaceState } from "@/lib/workspace/persistence";
...
```bash
bun test tests/workspace/reducer-fuzz.test.ts   # 60 seeds × 40 steps
bun tests/workspace/reducer-fuzz.ts             # full 300 seeds × 40 steps
```
